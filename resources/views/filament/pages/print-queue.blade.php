<x-filament-panels::page>
    ...
    <div class="rounded-xl bg-white shadow-sm ring-1 ring-gray-950/5 dark:bg-gray-900 dark:ring-white/10">
        @if ($queueItems->isEmpty())
            <div class="p-10 text-center text-gray-400 dark:text-gray-500">
                <x-filament::icon icon="heroicon-o-printer" class="mx-auto h-10 w-10 mb-3 opacity-40" />
                <p class="text-sm">Your queue is empty. Scan a product to get started.</p>
            </div>
        @else
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-gray-100 dark:border-white/10 text-left">
                            ...
                            <th class="px-4 py-3 font-semibold text-gray-700 dark:text-gray-200 text-center w-28">Copies</th>
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-white/5 transition">
                                ...
                                <td class="px-4 py-3 text-center">
                                    <input
                                        ...
                                        max="99"
                                        value="{{ $item->copies }}"
                                        wire:change="updateCopies({{ $item->id }}, $event.target.value)"
                                        class="w-16 rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-center text-sm px-2 py-1 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500"
                                    />
                                </td>
                                <td class="px-4 py-3">
                                    <button
    ...
    <script>
        document.addEventListener('livewire:initialized', () => {
            Livewire.on('open-pdf-export', ({ ids }) => {
                if (!ids || ids.length === 0) return;
                window.location.href = '/print/price-tags?ids=' + ids.join(',');
            });
            Livewire.on('open-pdf-preview', ({ ids }) => {
                if (!ids || ids.length === 0) return;
                window.open('/print/price-tags?ids=' + ids.join(',') + '&preview=1', '_blank');
            });
        });
    </script>
